...
munchStm :: (Monad m, MonadPut Instr m)
         => Supply Temp -> Stm -> ReaderT MipsFrame m ()
...
munchArgs :: (Monad m, MonadPut Instr m)
          => Supply Temp -> Int -> [Exp] -> ReaderT MipsFrame m [Temp]
munchArgs (S _ (S _ s1 s2) s3) n (e:es) = do
  args <- asks F.argRegs
  if n < length args
    then do
      let dst = args !! n
...
munchExp :: (Monad m, MonadPut Instr m)
         => Supply Temp -> Exp -> ReaderT MipsFrame m Temp
...
munchExp s e@(CallExp _ _) = munchStm s (ExpStm e) *> asks F.rv
...
newtype Gen a = Gen (State [Instr] a)
  deriving (Functor, Applicative, Monad)

execGen :: Gen a -> [Instr] -> [Instr]
execGen (Gen m) = execState m

instance MonadPut Instr Gen where
  put f = Gen $ modify' (f :)

codegen :: Supply Temp -> MipsFrame -> Stm -> [Instr]
codegen s f stm = reverse $ execGen (runReaderT (munchStm s stm) f) []